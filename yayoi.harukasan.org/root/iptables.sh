...
iptables_set_rules()
{
    ...
    do
#        iptables -t nat -A PREROUTING -i br0 -p tcp -d $server_ip --dport http -j DNAT --to 10.20.81.31:80      # Kibana Web Service
#        iptables -t nat -A PREROUTING -i br0 -p tcp -d $server_ip --dport 5044 -j DNAT --to 10.20.81.81:5044    # Logstash input port
